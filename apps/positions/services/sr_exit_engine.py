...
class MultiFactorSRCalculator:
    ...
    def _get_symbol(self) -> str:
        pos = self._position
        return getattr(pos, 'symbol', None) or getattr(pos, 'stock_code', 'NIFTY')

    def _get_current_price(self) -> float:
        return float(self._position.current_price or 0)

    def _get_pivot_sr(self):
        from apps.strategies.services.consolidated_sr_calculator import ConsolidatedSRCalculator
        result = ConsolidatedSRCalculator(self._get_symbol()).get_conservative_sr(self._get_current_price())
        if not result:
            return [], []
...
class SLTriggerChecker:
    ...
    def nearest_support_below(self) -> float | None:
        """Nearest support floor below current price (for ATR-buffered SL price)."""
    ...
    def nearest_resistance_above(self) -> float | None:
        """Nearest resistance ceiling above current price (for ATR-buffered SL price)."""
        price = float(self._pos.current_price or 0)
    ...
    def nearest_support_for_trigger(self) -> float | None:
        """
        Support closest to current price — may be above or below price.
        Used for the two-condition SL trigger check only.
        After a support break the relevant level is typically just above price.
        """
        price = float(self._pos.current_price or 0)
    ...
    def nearest_resistance_for_trigger(self) -> float | None:
        """
        Resistance closest to current price — may be above or below price.
        Used for the two-condition SL trigger check only.
        After a resistance break the relevant level is typically just below price.
        """
        price = float(self._pos.current_price or 0)
    ...
    def _get_oi_level(self, kind: str) -> float | None:
        ...
        try:
            ...
            symbol = getattr(self._pos, 'symbol', None) or 'NIFTY'
            price = float(self._pos.current_price or 0)
    ...
    def _condition_a_long(self, support: float) -> bool:
        """Price ≥ 0.5% below support."""
        return float(self._pos.current_price or 0) <= support * (1 - CONDITION_A_PCT)

    def _condition_b_long(self, support: float) -> bool:
        """Session low ≥ 1.0% below support."""
        low = self._tracker.get('session_low')
        return low is not None and float(low) <= support * (1 - CONDITION_B_PCT)

    def _condition_a_short(self, resistance: float) -> bool:
        """Price ≥ 0.5% above resistance."""
        return float(self._pos.current_price or 0) >= resistance * (1 + CONDITION_A_PCT)

    def _condition_b_short(self, resistance: float) -> bool:
        """Session high ≥ 1.0% above resistance."""
        high = self._tracker.get('session_high')
        return high is not None and float(high) >= resistance * (1 + CONDITION_B_PCT)
    ...
    def _check_long_sl(self) -> tuple:
        ...
        # fires when price is actually below the support (price ≤ support×0.995).
        level = self.nearest_support_for_trigger()
        if not level:
            return False, 'NO_SUPPORT_FOUND'
        if not self._condition_a_long(level):
            return False, 'CONDITION_A_NOT_MET'
        if not self._condition_b_long(level):
        ...
        return True, f'SR_SUPPORT_BREAK (support={level:.2f})'
    ...
    def _check_short_sl(self) -> tuple:
        ...
        # is actually above the resistance (price ≥ resistance×1.005).
        level = self.nearest_resistance_for_trigger()
        if not level:
            return False, 'NO_RESISTANCE_FOUND'
        if not self._condition_a_short(level):
            return False, 'CONDITION_A_NOT_MET'
        if not self._condition_b_short(level):
            return False, 'CONDITION_B_NOT_MET'
        if not self._not_recovering_15min(level, 'SHORT'):
            return False, 'PRICE_RECOVERING_15MIN'
        if not self._volume_confirmed():
            logger.info(f"SR SL: volume unconfirmed for pos #{self._pos.id} — proceeding")
        return True, f'SR_RESISTANCE_BREAK (resistance={level:.2f})'
...
class SRExitEngine:
    ...
    def evaluate(self) -> dict:
        result = {'sl_triggered': False, 'sl_reason': '', 'new_sl': None, 'new_target': None}
        ...
        if self._pos.direction == 'NEUTRAL':
            try:
                ...
                if prev_vix and prev_vix > 0 and (vix_now - prev_vix) / prev_vix > VIX_SPIKE_THRESHOLD:
                    result['sl_triggered'] = True
                    result['sl_reason'] = f'VOLATILITY_SPIKE (VIX {prev_vix:.1f}→{vix_now:.1f})'
                    self._tracker.save()
                    return result
            except Exception as e:
        ...
        # ── 3. Compute ATR-buffered SL price (direction-aware) ────────────────
        direction = self._pos.direction
        if direction == 'LONG':
            support = sl_checker.nearest_support_below()
            if support and support > 0:
                new_sl = support - 0.5 * atr
                if new_sl > 0:
...
def apply_sl_and_target(position, dashboard, now=None) -> dict:
    ...
    try:
        ...
        return {'sl_triggered': False, 'sl_reason': ''}
...
def compute_initial_sl_target(symbol: str, price: float, direction: str) -> dict:
    ...
    Args:
        ...
        price:     Current / entry price as float.
    ...
    try:
        ...
        mock_pos = _LightweightPos(symbol, float(price), direction)
        ...
        if direction == 'LONG':
            ...
            resistance = sl_checker.nearest_resistance_above()
            if resistance and resistance > 0:
                offset = interpolate_target_offset(atr / price if price else 0)
                t = resistance * (1 - offset)
                target = t if t > price else None
